...
Existing dynamic SD approaches follow predefined switching logic and remain confined to the SD execution path, introducing substantial residual overhead during pseudo-Non-SD generation, as demonstrated in~\autoref{tab:token_consistency_form} and~\autoref{fig:nonsd_overhead_b32}.
Therefore, \sysname exposes an externally controllable runtime interface that enables arbitrary switching logic and directly switches between native SD and Non-SD execution paths, avoiding the unnecessary overhead of Non-SD generation.

As shown in~\autoref{fig:sd_switch}, this design consists of three components: an external control interface, an iteration-level state transition in the scheduler, and path-specific execution paths in the worker.
...
\begin{figure}[b]
    ...
    \begin{minipage}[t]{0.73\linewidth}
        \centering
        \captionof{table}{Execution path comparison across adaptive SD approaches.}
        ...
        \begin{tabular}{ccc}
            \toprule
            \textbf{Approach} &
            \makecell{\textbf{Non-SD Path}} &
            \makecell{\textbf{SD Path}} \\
            \midrule
        
            \makecell[l]{vLLM Dynamic SD ($\text{draft\_len}=0$)}
            & $\times$ & $\checkmark$ \\
        
            \makecell[l]{SGLang Adaptive SD ($\text{draft\_len}=0$)}
            & $\times$ & $\checkmark$ \\
...
% 外部接口
\textbf{External Control Interface for Better Extendibility.}
...
\sysname extends the inference engine with a lightweight control interface that receives SD switching requests and forwards them to the scheduler to update the desired execution mode.
This design decouples external control policies from internal execution logic, allowing switching policies to be modified or replaced independently of the inference engine.
...
\sysname further parses the prompt for state-indicative keywords related to errors, repeated actions, completion, and termination, and maps the request to a workflow-specific execution state.
...
Type A directly leverages the explicit plan, Type B uses the most frequent historical completion step to capture recurring termination patterns, Type C further incorporates state transitions and uses the median for more robust estimation.
...
\label{sec:4.3.1}
For each model and hardware configuration, \sysname profiles the relationship between serving load and SD speedup, and identifies a break-even load $B^{*}$.
