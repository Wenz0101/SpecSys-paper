...
Although some existing methods allow the draft length to be reduced to zero, this pseudo-Non-SD mode may still retain SD overhead. 
...
% 说明我们与他们的不同
% 与 LLM 层面的预测和调度的不同
On the one hand, compared with request-level scheduling methods that estimate the remaining generation length of individual LLM requests, SpecSys operates at a higher granularity by estimating the remaining steps of an entire agent task across multiple LLM calls.
% 与 Agent 层面的预测和调度的不同
