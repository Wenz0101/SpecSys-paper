...
In contrast, SpecSys decides whether to enable SD, switches directly between native SD and non-SD paths, and exposes a runtime interface for arbitrary upper-layer control.


% 介绍 agent 与 SD 结合的论文，说明我们跟他们的区别。
Beyond optimizing SD within token-level generation, recent work extends speculative decoding from token-level generation to semantic-level reasoning~\citep{SpecReason,DREAM-R}, and further applies this paradigm to agent planning, action, or tool execution~\citep{Interactive_Speculative_Planning,Dynamic_Speculative_Agent_Planning,Speculative_Actions}.
In contrast, these approaches primarily redesign the upper-level agent workflow, whereas SpecSys applies speculative decoding to the LLM requests within each agent job to accelerate inference without altering the agent structure, making it orthogonal to these approaches.
% 简单阐述我们的方法理论上可以拓展到目前新兴的 speculative agent workflows
In principle, our remaining-step prediction method could also be extended to such emerging speculative agent workflows by simply scaling the predicted number of remaining steps according to the speculative acceptance rate.
...
% SpecSys operates at a granularity of the agent task by estimating the remaining steps of an entire agent task across multiple LLM calls \xiaoxi{why task level? as optimizing independently for individual steps might not be the global optimum in reducing the total JCT in the system.}.
% % 与 Agent 层面的预测和调度的不同
% Other agent schedulers \xiaoxi{cite} mainly rely on observed execution states or predictions of near-term execution stages, rather than explicitly estimating the remaining work of the entire agent task. SpecSys instead estimates the remaining number of agent steps and uses it to prioritize tasks closer to completion, \xiaoxirev{which follows the shortest-remaining-time-first principle to better approach the global optimum in JCT minimization.}

Existing LLM schedulers optimize individual LLM requests through predicted output lengths or relative length rankings~\citep{SSJF,LTR}, online estimation of the remaining generation length~\citep{TRAIL}, preemptive queue-based scheduling~\citep{FastServe}, or memory-aware and SLO-aware scheduling based on estimated future resource demands~\citep{Past-Future,JITServe}. These methods operate primarily at the granularity of individual LLM calls. In contrast, SpecSys operates at the granularity of the entire agent task\footnote{In this work, we use `agent task' and `agent request' interchangeably.} by estimating its remaining steps across multiple LLM calls, since independently prioritizing individual calls may not align with minimizing the end-to-end JCT of agent tasks.

Existing agent schedulers exploit application-level dependencies and dataflows~\citep{Parrot}, completed-call history or attained service~\citep{Agentix}, historical execution states and predicted future stages~\citep{Astraea}, program-level execution and resource states~\citep{ThunderAgent}, or workflow structure and cache information~\citep{Helium} to guide scheduling. However, they do not explicitly estimate the remaining work of an entire agent task. SpecSys instead estimates the remaining number of agent steps and prioritizes tasks closer to completion, following the shortest-remaining-time-first principle at the agent-task level to better align scheduling decisions with end-to-end JCT minimization.
