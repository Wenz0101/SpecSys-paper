...
\begin{itemize}
    % Non-SD
    \item \textbf{Non-SD execution mode}.
    \texttt{Execute\_Model} performs a standard LLM forward pass with a
    \textbf{decode kernel}, producing next-token logits for each request.
    \texttt{Sample\_Tokens} then samples one token from these logits.

    % SD
    \item \textbf{SD execution mode}.
    \texttt{Execute\_Model} verifies multiple draft tokens in parallel using a
    \textbf{prefill-style kernel} for multi-token queries.
    \texttt{Sample\_Tokens} then invokes the drafter to generate tokens for
